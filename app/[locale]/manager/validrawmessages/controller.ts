...
export const controller = () => {
  ...
  const [dataDatatypeList, setDataDatatypeList] = useState([]);
  ...
  useEffect(() => {
    const refetchData = async () => {
      if (!user.token || !selectedCustomerId) {
        setDataObjectList([]);
        setDataDatatypeList([]);
        setLoading(false);
        return;
      ...
      setLoading(true);
      try {
        const [objectListData, datatypeListData] = await Promise.all([
          objectList(getUserRef().token),
          datatypeList(getUserRef().token)
        ]);
        
        setDataObjectList(objectListData);
        setDataDatatypeList(datatypeListData);
      } catch (error) {
