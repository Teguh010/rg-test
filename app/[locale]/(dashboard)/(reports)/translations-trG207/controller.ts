...
export const controller = () => {
  ...
  useEffect(() => {
    const fetchTranslations = async () => {
      setIsLoading(true);
      try {
        const result = await translationList(user?.token || null, currentLanguage);
  ...
  const updateTranslation = async (translations: Translation | Translation[]) => {
    try {
      ...
      const result = await translationSetBulk(user?.token || null, currentLanguage, formattedTranslations);
  ...
  const addTranslations = async () => {
    ...
    try {
      const result = await translationSetBulk(user?.token || null, currentLanguage, newTranslations);
  ...
  const submitUploadedTranslations = async () => {
    ...
    try {
      const result = await translationSetBulk(user?.token || null, currentLanguage, uploadedTranslations);
  ...
  return {
    models: {
      ...
      isSaving,
      currentLanguage,
      availableLanguages: ['en', 'es', 'fr', 'pt', 'ru', 'lt', 'no'],
      isLoading,
      translationInputs,
      referenceLanguage,
      missingKeys,
