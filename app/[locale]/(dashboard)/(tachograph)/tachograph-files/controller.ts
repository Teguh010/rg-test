  ...
  const filterData = (objects: any[]) => {
    return objects.map(obj => {
      ...
      if (newObj.data_timestamp) {
        const time = format(newObj.data_timestamp, `${dateFormat} ${timeFormat}`);
      ...
      if (user.token) {
        (async () => {
          try {
            ...
            setDataTachoFilesList(dataTachoFilesListTranslate);
            ...
            setDataTachoDriverCardFilesList(dataTachoDriverCardFilesListTranslate);
            setLoading(false);
          } catch (error) {
