...
export const serverFetchAddresses = async (token, objects) => {
  if (objects) {
    ...
    try {
        ...
        objects.map(async (obj) => {
          ...
          if (obj.lat && obj.lon && !obj.address) {
            ...
            if (address) {
              ...
              const { address } = await serverFetchAddress(obj.lat, obj.lon);
              newAddresses.push({ lat: obj.lat, lng: obj.lon, a: address?.label });
              updatedObj.from = address?.label;
            }
          } else {
          ...
          if (obj.next_lat && obj.next_lon && !obj.next_address) {
            ...
            if (address) {
              ...
              const { address } = await serverFetchAddress(obj.next_lat, obj.next_lon);
              newAddresses.push({ lat: obj.next_lat, lng: obj.next_lon, a: address?.label });
              updatedObj.to = address?.label;
            }
          } else {
      ...
      if (newAddresses.length >= 1) {
        await serverAddressCacheAdd(token, newAddresses);
      }
