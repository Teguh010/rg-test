import 'jspdf-autotable'

const formatTitle = (text) => {
  return text
    .split(/[_\s]/)
    .map((word) => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase())
    .join(' ')
}

const formatDate = (dateString, format = 'dd/MM/yyyy') => {
  if (!dateString) return '-';
  const date = new Date(dateString);
  
  if (format === 'dd/MM/yyyy') {
    return `${date.getDate().toString().padStart(2, '0')}/${(date.getMonth() + 1).toString().padStart(2, '0')}/${date.getFullYear()}`;
  }
  
  return date.toLocaleDateString('en-GB');
};

export const generateTripStopPDF = ({
  doc,
  data,
  totals,
  t = (key) => key,
  dateFormat = 'dd/MM/yyyy',
  columns = null,
  unitDistance = 'km'
}) => {
  try {
    const logoBase64 = 'data:image/png;base64,';
    // Add logo and title
    doc.addImage(logoBase64, 'PNG', 10, 5, 35, 15)
    doc.setFontSize(20)
    doc.setFont('helvetica', 'bold')
    doc.text('Trip Stop Report', 140, 20, { align: 'center' })
    
    // Add date range
    doc.setFontSize(12)
    doc.setFont('helvetica', 'normal')
    const formattedDate = new Date().toLocaleDateString('en-GB')
    doc.text(`Date Range: ${formattedDate}`, 140, 30, { align: 'center' })

    let yTotals = 45; 

    // 1. Buat halaman khusus TOC terlebih dahulu
    const toc = []
    let yPos = 60
    
    // 2. Simpan halaman TOC
    const tocPage = doc.internal.getCurrentPageInfo().pageNumber
    
    const outline = doc.outline;
    const rootOutline = outline ? outline : {};

    // 4. Tambahkan bookmark TOC
    if (typeof rootOutline.add === 'function') {
      rootOutline.add(null, 'Table of Contents', { pageNumber: tocPage });
    }

    // 5. Generate konten kendaraan di halaman baru
    data.forEach((vehicle, index) => {
      doc.addPage()
      const currentPage = doc.internal.getCurrentPageInfo().pageNumber
      toc.push({ 
        id: vehicle.objectid, 
        page: currentPage,
        name: vehicle.vehicleName || `Vehicle ${index + 1}`
      })

      if (typeof rootOutline.add === 'function') {
        const vehicleBookmark = rootOutline.add(
          null, 
          vehicle.vehicleName || `Vehicle ${index + 1}`,
          { pageNumber: currentPage, y: 20 }
        );

        if (vehicle.d && vehicle.d.length > 0) {
          rootOutline.add(
            vehicleBookmark,
            'Detail Perjalanan',
            { pageNumber: currentPage, y: yTotals + 10 }
          );
        }
      }

      // Vehicle header
      doc.setFontSize(16)
      doc.setFont('helvetica', 'bold')
      doc.text(`Vehicle: ${vehicle.vehicleName || 'Unknown'}`, 10, 20)

      // Vehicle summary
      const summary = calculateVehicleSummary(vehicle.d)
      doc.setFontSize(12)
      doc.setFont('helvetica', 'normal')
      doc.text(`Total Distance: ${summary.totalDistance}`, 10, 30)
      doc.text(`Total Fuel Used: ${summary.totalFuel}`, 10, 35)
      doc.text(`Total Duration: ${summary.totalDuration}`, 10, 40)

      // Trip data table
      if (vehicle.d && vehicle.d.length > 0) {
        doc.autoTable({
          startY: yTotals + 10,
          head: [
            columns
              ? columns.map(col => col.header)
              : [
                  'State',         
                  'From',         
                  'To',           
                  'Time From',    
                  'Time To',      
                  'Duration',     
                  'Distance',     
                  'Worker',       
                  'Avg Speed'     
                ]
          ],
          body: vehicle.d.map(trip => [
            trip.state || '-',
            trip.address || '-',
            trip.next_address 
              ? trip.next_address 
              : (trip.next_lat && trip.next_lon 
                  ? `[Koordinat] ${trip.next_lat}, ${trip.next_lon}` 
                  : '-'),
            formatDate(trip.time_from, dateFormat),
            formatDate(trip.time_to, dateFormat),
            trip.duration || '-',
            `${trip.distance || '0'} ${unitDistance}`,
            trip.worker || '-',
            `${trip.avg_speed || '0'} ${unitDistance}/h`
          ]),
          theme: 'striped',
          styles: {
            fontSize: 8,
            cellPadding: 3,
            overflow: 'linebreak',
            halign: 'left',
            fillColor: [255, 255, 255]
          },
          headStyles: {
            fillColor: [33, 150, 243], // Warna biru sama
            textColor: [255, 255, 255],
            fontStyle: 'bold',
            halign: 'left'
          },
          columnStyles: {
            0: { cellWidth: 20 },  // State
            1: { cellWidth: 'auto' }, // From
            2: { cellWidth: 'auto' }, // To
            3: { cellWidth: 25 }, // Time From
            4: { cellWidth: 25 }, // Time To
            5: { cellWidth: 20 }, // Duration
            6: { cellWidth: 20 }, // Distance
            7: { cellWidth: 25 }, // Worker
            8: { cellWidth: 25 }  // Avg Speed
          },
          margin: { top: 10, left: 10, right: 10, bottom: 15 },
          bodyStyles: { fontStyle: 'bold' },
          alternateRowStyles: {
            fillColor: [240, 240, 240]
          }
        })
      }

      // Tambahkan pengecekan posisi Y
      const maxY = doc.internal.pageSize.height - 20;
      if (yPos > maxY) {
        doc.addPage();
        yPos = 20; // Reset Y position
      }
    })

    if (doc.outline) {
      doc.outline.style = 'bold';
      doc.outline.color = [33, 150, 243];
    }

    // Add page numbers
    const pageCount = doc.internal.getNumberOfPages()
    for (let i = 1; i <= pageCount; i++) {
      doc.setPage(i)
      doc.setFontSize(8)
      doc.text(
        `Page ${i} of ${pageCount}`,
        doc.internal.pageSize.width - 20,
        doc.internal.pageSize.height - 10,
        { align: 'right' }
      )
    }

    // 1. Generate TOC Content
    doc.setPage(tocPage)
    doc.setFontSize(12)
    doc.text('Table of Contents', 10, 30)
    
    toc.forEach((item, index) => {
      if (yPos > doc.internal.pageSize.height - 20) {
        doc.addPage()
        yPos = 20
      }
      
      // Add TOC entry text
      doc.text(`${item.name}`, 10, yPos)
      doc.text(`... ${item.page}`, doc.internal.pageSize.width - 20, yPos, { align: 'right' })
      
      // Add internal link
      doc.link(10, yPos - 5, doc.internal.pageSize.width - 20, 10, {
        pageNumber: item.page
      })

      yPos += 10
    })

    // 2. Bookmark Configuration
    if (doc.outline) {
      // Initialize outline items array if not exists
      if (!doc.outline._items) {
        doc.outline._items = [];
      }
      
      // Add root outline item if empty
      if (doc.outline._items.length === 0) {
        doc.outline.add(null, 'Root', { pageNumber: 1 });
      }

      // Set root reference safely
      doc.outline._root = doc.outline._items[0] || null;

      // Set viewer preferences
      if (doc.viewerPreferences) {
        doc.viewerPreferences({
          HideToolbar: false,
          HideMenubar: false,
          HideWindowUI: false,
          DisplayDocTitle: true,
          NonFullScreenPageMode: 'UseOutlines',
          PageMode: 'UseOutlines'
        });
      }
    
    }

    return doc
  } catch (error) {
    console.error('PDF Generation error:', error)
    throw error
  }
}

// Helper function to calculate vehicle summary
const calculateVehicleSummary = (trips) => {
  const parseDuration = (duration) => {
    if (!duration) return 0;
    
    // Handle format "X days HH:mm:ss"
    const daysMatch = duration.match(/(\d+)\s*days?/);
    const timeMatch = duration.match(/(\d{1,2}:\d{2}:\d{2})/);
    
    let totalSeconds = 0;
    
    if (daysMatch) {
      totalSeconds += parseInt(daysMatch[1]) * 86400;
    }
    
    if (timeMatch) {
      const [hours, minutes, seconds] = timeMatch[1].split(':').map(Number);
      totalSeconds += hours * 3600 + minutes * 60 + seconds;
    }
    
    return totalSeconds;
  };

  const formatDuration = (totalSeconds) => {
    const days = Math.floor(totalSeconds / 86400);
    const hours = Math.floor((totalSeconds % 86400) / 3600);
    const minutes = Math.floor((totalSeconds % 3600) / 60);
    const seconds = Math.floor(totalSeconds % 60);
    
    return `${days > 0 ? `${days}d ` : ''}${String(hours).padStart(2, '0')}:${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')}`;
  };

  const total = trips.reduce((acc, trip) => {
    const durationSeconds = parseDuration(trip.duration);
    return {
      totalDistance: acc.totalDistance + (parseFloat(trip.distance) || 0),
      totalFuel: acc.totalFuel + (parseFloat(trip.fuel_used) || 0),
      totalSeconds: acc.totalSeconds + durationSeconds
    };
  }, { totalDistance: 0, totalFuel: 0, totalSeconds: 0 });

  return {
    totalDistance: total.totalDistance.toFixed(3) + ' km',
    totalFuel: total.totalFuel.toFixed(3) + ' L',
    totalDuration: formatDuration(total.totalSeconds)
  };
};
